...
const tableaux = [tableau1, tableau2, tableau3];

button.addEventListener("click", () => {
  // On choisit le tableau correspondant au clic (1 → tableau1, 2 → tableau2, 3 → tableau3)
  const indexTableau = (clics - 1) % tableaux.length;
  const tableau = tableaux[indexTableau];

  // Tirage aléatoire dans ce tableau
  const choix = tableau[Math.floor(Math.random() * tableau.length)];

  // Affichage
  if (clics === 1) {
    outputDiv.innerHTML = `<p>${choix}</p>`;
  } else {
    outputDiv.innerHTML += `<p>${choix}</p>`;
  }

  clics++;
  if (clics > tableaux.length) clics = 1;
});
